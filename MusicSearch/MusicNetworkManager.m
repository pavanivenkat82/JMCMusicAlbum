...
@interface CEONetworkManager()<NSURLSessionDelegate,NSURLSessionDataDelegate>
@property (nonatomic) NSMutableData* responseData;
@property (nonatomic) CEOMRequestData* reqData;
@end
@implementation CEONetworkManager
-(void) setPostParametersForRequest:(NSMutableURLRequest*)request requestData:(CEOMRequestData*) reqData
{
    
    [request setValue:@"application/x-www-form-urlencoded; charset=utf-8" forHTTPHeaderField:@"Content-Type"];
    if(!reqData.inputParams || [reqData.inputParams count] == 0)
    {
        if(reqData.isPostData)
        {
            [request setURL:[NSURL URLWithString: reqData.url]];
            if(reqData.postData && [reqData.postData length] !=0)
                [request setHTTPBody:reqData.postData ];
    ...
    if(reqData.networkRequestType == CEONetworkRequestTypePOST)
    {
...
-(void) setHeadersForRequest:(NSMutableURLRequest*)request requestData:(CEOMRequestData*) reqData
{
    if(reqData)
    {
        [reqData.additionalHeaders enumerateKeysAndObjectsUsingBlock:^(NSString* key, NSString* value, BOOL *stop) {
            DLog(@"Settin header Key %@ Value %@",key,value);
...
-(void) sendAsynchronousRequestWithCustomDelegate:(CEOMRequestData*) requestData
{
    ...
    configuration.HTTPAdditionalHeaders = requestData.additionalHeaders;
    ...
    //    [request setValue:@"application/x-www-form-urlencoded; charset=utf-8" forHTTPHeaderField:@"Content-Type"];
    NSURLSession *session = [NSURLSession sessionWithConfiguration:configuration delegate:self delegateQueue:[CEONetworkManager networkQueue]];
...
-(void) sendAsynchronousRequest:(CEOMRequestData*) requestData
{
    ...
    NSURLSessionDataTask *task = [session dataTaskWithRequest:request
                                  ...
                                  ^(NSData *data, NSURLResponse *response, NSError *error)
                                  {
                                      ...
                                      DLog(@"Response Data %@",myString);
                                      //                                      NSDictionary* responseDict = [NSJSONSerialization JSONObjectWithData:data options:NSJSONReadingMutableContainers error:&error];
                                      NSDictionary* responseDict = [CEOJSONUntil convertJSONDataToDictionary:data];
                                      
                                      CEOMResponseData* responseData = [[CEOMResponseData alloc] initWithDictionary:responseDict serviceType:requestData.serviceType];
...
didCompleteWithError:(NSError *)error
{
    ...
    else
    {
        ...
        NSLog(@"Response %@",str);
        NSDictionary* responseDict = [CEOJSONUntil convertJSONDataToDictionary:_responseData];
        
        CEOMResponseData* responseData = [[CEOMResponseData alloc] initWithDictionary:responseDict serviceType:_reqData.serviceType];
        if(responseData)
